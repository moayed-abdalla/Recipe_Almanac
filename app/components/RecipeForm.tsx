 ...
 * 
 * Shared form component for both creating and editing recipes.
 * When initialRecipe is provided, it operates in edit mode.
 * Otherwise, it operates in create mode.
 * 
 * Features:
 * - Title, description, and tags
 * - Recipe image upload (with preview for edit mode)
...
export default function RecipeForm({ initialRecipe, initialIngredients = [] }: RecipeFormProps) {
  ...
  const [isPublic, setIsPublic] = useState(initialRecipe?.is_public ?? true);
  
  // Form state - recipe content (pre-filled if editing)
  const [methodSteps, setMethodSteps] = useState<string[]>(
    initialRecipe?.method_steps.length ? initialRecipe.method_steps : ['']
  );
  const [notes, setNotes] = useState<string[]>(
    initialRecipe?.notes.length ? initialRecipe.notes : ['']
  );
  ...
  const originalImageUrl = initialRecipe?.image_url || null;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Step 2: Verify ownership if editing
      if (isEditMode && user.id !== initialRecipe!.user_id) {
        throw new Error('You do not have permission to edit this recipe');
      }

      // Step 3: Generate URL-friendly slug from title
      const slug = title
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/(^-|-$)/g, '');

      // Step 4: Upload image to Supabase Storage if provided
      let imageUrl = originalImageUrl; // Keep existing image by default in edit mode
      if (imageFile) {
        try {
      ...
      if (isEditMode) {
        // UPDATE MODE: Update existing recipe
        ...
        <div className="form-control mt-6">
          <div className="flex gap-2">
            ...
            {isEditMode && (
              <button
                type="button"
                onClick={() => router.push(`/recipe/${initialRecipe!.id}`)}
